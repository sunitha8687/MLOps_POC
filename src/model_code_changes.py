...
# put into an abstract class or  a parent class.
...
# inherit from preprocessing into train and then again into predict into
...
# Classification of the latest sensor data, Predict labels and plot the inference


def predict_probab(inference_df, cm, cm_bright, xx, yy, Z):
    """
...
if __name__ == '__main__':
    ...
    pred_results = predict_probab(inference_df, cm, cm_bright, xx, yy, Z)
    f1_score, accuracy_score, precision, recall = predict_evaluate(df)
    mlflow.log_param('kernel', 1.0 * RBF(1.0)) # try to wrap in utils.
